...
from ducklake_serverless.config import get_settings


def test_required_env_loads(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("DUCKLAKE_SERVERLESS_HEALTH_CHECK_URL", "https://svc.example")
...
def test_missing_required_env_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("DUCKLAKE_SERVERLESS_HEALTH_CHECK_URL", raising=False)
...
def test_log_level_validates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("DUCKLAKE_SERVERLESS_HEALTH_CHECK_URL", "https://svc.example")
    monkeypatch.setenv("DUCKLAKE_SERVERLESS_LOG_LEVEL", "verbose")
    with pytest.raises(ValidationError):
        get_settings()
