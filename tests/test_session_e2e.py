...
def test_bootstrap_twice_fails(lake: Lake) -> None:
    lake.bootstrap()
...
def test_lost_cas_race_aborts_cleanly(lake: Lake, store: InMemoryObjectStore) -> None:
    """Another writer advances the root mid-transaction: clean abort (P1)."""
    lake.bootstrap()
    base, etag = read_root(store)

    with pytest.raises(ConflictAbortError), lake.transaction() as tx:
        tx.sql("CREATE TABLE mine (v INTEGER)")
        # Simulate a rival committing while our transaction is open. The
        # rival's generation must exist for post-abort readers to resolve.
        rival = base.model_copy(update={"generation": 1})
        store.put_if_absent(
            format_catalog_key(1, rival.catalog_uuid),
            store.get(base.catalog_key).body,
        )
        publish_root(store, rival, etag)

    doc, _ = read_root(store)
    assert doc.generation == 1  # the rival's commit, not ours


def test_version_mismatch_refused(lake: Lake, store: InMemoryObjectStore) -> None:
    lake.bootstrap()
    doc, etag = read_root(store)
    pinned = doc.model_copy(update={"duckdb_storage_version": "v0.0.1-other"})
    publish_root(store, pinned, etag)
